...
__check_online(player_name) ->(
	...
	if (f_player,
		if (query(f_player,'has_tag',global_tag), 
			exit('§4假人'+f_player+'已经在线了');
			''
		);
		exit('§4玩家'+f_player+'已经在线了');
		''
	)
);
__check_pos(x) ->(
...
reload() ->(
	...
	run(str('tellraw @a {"text":"tagplayer重载成功！"}'));
	''
);
summon(player_name) ->(
...
killall() ->(
	...
	print('已将全部假人移交§6FZ\'sDataPack§f清除标签');
	game_tick(50);
	print('已清除全部假人');
	''
);
